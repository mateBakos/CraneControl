% Ensemble control for a crane system
% Nathan Vervaeke
% Universiteit Gent
% 2014
...
% Ensemble parameters
order = 2;                 % Number of steps of the ensemble control

% Simulation parameters
stepsim = 0.016;           % Simulation step size (in seconds)
ts = 30*stepsim;           % Step size (in seconds)
...
% Calculate input
input = CalculateInputCrane(n,order,start,final,Ad,Bd,theta_epsilon_final,nominaleps);
...
xlabel('Time [s]')
title(['x_{start} = ',num2str(x_start),'    x_{final} = ',num2str(x_final)])
subplot(2,2,2)
hold on
...
set(get(AX(2),'Ylabel'),'String','Acceleration [m/s²]')
xlabel('Time [s]')
title(['dx/dt_{start} = ',num2str(dxdt_start),'    dx/dt_{final} = ',num2str(dxdt_final)])
subplot(2,2,3)
hold on
...
set(get(AX(2),'Ylabel'),'String','Acceleration [m/s²]')
xlabel('Time [s]')
title(['\theta_{start} = ',num2str(theta_start),'    \theta_{final} = ',num2str(theta_final)])
subplot(2,2,4)
hold on
...
set(get(AX(2),'Ylabel'),'String','Acceleration [m/s²]')
xlabel('Time [s]')
title(['d\theta/dt_{start} = ',num2str(dthetadt_start),'    d\theta/dt_{final} = ',num2str(dthetadt_final)])