% Three-phase ensemble control for a crane system
% Nathan Vervaeke
% Universiteit Gent
% 2014
...
x_intermediate1 = 0.1;
dxdt_intermediate1 = 0;
theta_intermediate1 = 0.1;
theta_epsilon_intermediate1 = 0;
dthetadt_intermediate1 = -0.1;

x_intermediate2 = -0.1;
dxdt_intermediate2 = 0;
theta_intermediate2 = -0.1;
theta_epsilon_intermediate2 = 0;
dthetadt_intermediate2 = 0.1;
...
intermediate1_1 = [x_intermediate1,dxdt_intermediate1,theta_intermediate1,dthetadt_intermediate1]; % Intermediate states
intermediate1_2 = [x_intermediate1,dxdt_intermediate1,theta_intermediate1+theta_epsilon_intermediate1*(epsilon-nominaleps),dthetadt_intermediate1]; % Intermediate states
intermediate2_1 = [x_intermediate2,dxdt_intermediate2,theta_intermediate2,dthetadt_intermediate2]; % Intermediate states
intermediate2_2 = [x_intermediate2,dxdt_intermediate2,theta_intermediate2+theta_epsilon_intermediate2*(epsilon-nominaleps),dthetadt_intermediate2]; % Intermediate states
...
% Calculate input
input1 = CalculateInputCrane(n,order,start,intermediate1_1,Ad,Bd,theta_epsilon_intermediate1,nominaleps);
input2 = CalculateInputCrane(n,order,intermediate1_2,intermediate2_1,Ad,Bd,theta_epsilon_intermediate2,nominaleps);
input3 = CalculateInputCrane(n,order,intermediate2_2,final,Ad,Bd,theta_epsilon_final,nominaleps);
...
for i = 1:length(input)
    for j = 1:(T/stepsim)/length(input)
        in((i-1)*ts/stepsim+j) = real(input(i));
        fill = (i-1)*ts/stepsim+j;
    end
end
...
subplot(2,2,1)
hold on
plot(t,x_intermediate1)
hold on
plot(t,x_intermediate2)
hold on
[AX] = plotyy(t,x_final,u(:,1),u(:,2));
set(get(AX(1),'Ylabel'),'String','Distance [m]')
set(get(AX(2),'Ylabel'),'String','Acceleration [m/s²]')
xlabel('Time [s]')
title(['x_{start} = ',num2str(x_start),'    x_{intermediate} = {',num2str(x_intermediate1),', ',num2str(x_intermediate2),'}    x_{final} = ',num2str(x_final)])
subplot(2,2,2)
hold on
plot(t,dxdt_intermediate1)
hold on
plot(t,dxdt_intermediate2)
hold on
[AX] = plotyy(t,dxdt_final,u(:,1),u(:,2));
set(get(AX(1),'Ylabel'),'String','Speed [m/s]')
set(get(AX(2),'Ylabel'),'String','Acceleration [m/s²]')
xlabel('Time [s]')
title(['dx/dt_{start} = ',num2str(dxdt_start),'    dx/dt_{intermediate} = {',num2str(dxdt_intermediate1),', ',num2str(dxdt_intermediate2),'}    dx/dt_{final} = ',num2str(dxdt_final)])
subplot(2,2,3)
hold on
plot(t,theta_intermediate1)
hold on
plot(t,theta_intermediate2)
hold on
[AX] = plotyy(t,theta_final,u(:,1),u(:,2));
set(get(AX(1),'Ylabel'),'String','Angle [rad]')
set(get(AX(2),'Ylabel'),'String','Acceleration [m/s²]')
xlabel('Time [s]')
title(['\theta_{start} = ',num2str(theta_start),'    \theta_{intermediate} = {',num2str(theta_intermediate1),', ',num2str(theta_intermediate2),'}    \theta_{final} = ',num2str(theta_final)])
subplot(2,2,4)
hold on
plot(t,dthetadt_intermediate1)
hold on
plot(t,dthetadt_intermediate2)
hold on
[AX] = plotyy(t,dthetadt_final,u(:,1),u(:,2));
set(get(AX(1),'Ylabel'),'String','Angular speed [rad/s]')
set(get(AX(2),'Ylabel'),'String','Acceleration [m/s²]')
xlabel('Time [s]')
title(['d\theta/dt_{start} = ',num2str(dthetadt_start),'    d\theta/dt_{intermediate} = {',num2str(dthetadt_intermediate1),', ',num2str(dthetadt_intermediate2),'}    d\theta/dt_{final} = ',num2str(dthetadt_final)])